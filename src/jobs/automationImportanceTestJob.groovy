    ...
    jobCFG.each { jobName, config ->
        pipelineJob("sat-${versionName}-${jobName}-tests") {
            ...
            parameters {
                ...
                stringParam('sat_version', "", "Satelittle version to deployed, format is a.b.c")
                stringParam('appliance_count', "${config['num_appliances']}", "Number of appliances to run each tier stage against")
                stringParam('tower_url', globalJenkinsDefaults.tower_url, "Ansible Tower URL, format 'https://<url>/'")
                stringParam('importance', "${config['importance']}", "Importance mark for pytest session. Must be capitalized")
            }
